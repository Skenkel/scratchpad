...
num_summoners = sum(1 for line in open('friends.txt'))
#print(num_summoners)
summonersraw = open("friends.txt")
#print(apikey)
summonerstaticlist = []
...
# Now we print out our results
    for tempiter in range(0, num_summoners):
        ...
        if tempiter == 0:
            msg = 'To:9195649288@tmomail.net\n\n From:lolstalker \n\nLeague Status:   '
        if summonerdynamiclist[tempiter][0] == 0:
            #print("at least one account is in game")

            #message = repr(summonerstaticlist[tempiter][1])) + 'is not in game right now. The account is level'+repr(summonerdynamiclist[tempiter[2]])+"In the"+repr(summonerdynamiclist[tempiter[3]]) +"league, division"+repr(summonerdynamiclist[tempiter[4]]) + "with"+ repr(summonerdynamiclist[tempiter[5]]) +"lp")
            if summonerdynamiclist[tempiter][6] == 1:
                ...
                print("\n")
                 ...
                 message = repr(summonerstaticlist[tempiter][2]) + "account is not in game right now. The account is level " + repr(summonerdynamiclist[tempiter][2]) + " and is unranked"
                 print(message)
                 print("\n")
        else:
            if summonerdynamiclist[tempiter][6] == 1:
                message = repr(summonerstaticlist[tempiter][2]) + "account is in game right now. Playin champion id" +repr(summonerdynamiclist[tempiter][1])+ "The account is level " + repr(summonerdynamiclist[tempiter][2])+ " and is in the" + repr(summonerdynamiclist[tempiter][3])+ "league, division" + repr(summonerdynamiclist[tempiter][4])+ "with" + repr(summonerdynamiclist[tempiter][5])+"lp" 
                msg = msg + message
                print(message)
                print("\n")

            else:
                 message = repr(summonerstaticlist[tempiter][2]) + "account is in game right now. The account is level " + repr(summonerdynamiclist[tempiter][2]) + " and is unranked"
                 msg = msg+ message
                 print(message)
                 print("\n")
        if tempiter == (num_summoners-1) :
            print("final loop about to send text message")
            if msg == 'To:9195649288@tmomail.net\n\n From:lolstalker \n\nLeague Status:   ':
                msg = msg + 'No one is playing league'
            server = smtplib.SMTP( "smtp.gmail.com", 587 )
            server.starttls()
            server.login( 'CHANGEME', 'CHANGEME' )
            #curtime = datetime.datetime.now()
            #curhour = curtime[3]
            #if sendhour != curhour:
            #    server.sendmail( 'lolstalker', '9195649288@tmomail.net', msg)
            #    sendhour = curhour
            msg = msg+ "\n\n"
